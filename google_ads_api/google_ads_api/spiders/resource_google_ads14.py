...
class ResourceGoogleAdsSpider(scrapy.Spider):
    name = "resource_google_ads14"
    allowed_domains = ["developers.google.com"]
    start_urls = ["https://developers.google.com/google-ads/api/fields/v14/overview"]

    def parse(self, response):
    # Loop through each element from nth-child(3) to nth-child(59)    
        for i in range(3, 60):
        # Create a selector for each element
            selector = f"body > section > devsite-book-nav > nav > div.devsite-book-nav-wrapper > div.devsite-mobile-nav-bottom > ul > li > div > ul > li:nth-child(2) > div > ul > li:nth-child(3) > div > ul > li:nth-child({i}) > a"
    ...
    def parse_resource(self, response):
        ...
        # Create a selector for each element
        selector2 = f"body > section > devsite-book-nav > nav > div.devsite-book-nav-wrapper > div.devsite-mobile-nav-bottom > ul > li > div > ul > li:nth-child(2) > div > ul > li:nth-child(3) > div > ul > li:nth-child({i}) > a > span"
